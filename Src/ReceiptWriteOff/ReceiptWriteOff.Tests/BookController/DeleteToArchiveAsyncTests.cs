...
{
    [Fact]
    public async Task DeleteToArchiveAsync_DeletesBook()
    {
        // Arrange
        int id = 1;
        ...
        // Act
        var result = await model.Controller.DeleteToArchiveAsync(id, CancellationToken.None);

        // Assert
        result.Should().BeOfType<OkResult>();
        model.ServiceMock.Verify(
            service => service.DeleteToArchiveAsync(id, CancellationToken.None),
            Times.Once);
    }

    [Fact]
    public async Task DeleteToArchiveAsync_ReturnsNotFound_WhenBookNotFound()
    {
        // Arrange
        int id = 1;
